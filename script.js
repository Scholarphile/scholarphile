...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function hydrateFromApi() {
        ...
        try {
            ...
            // Render documents
            const dgrid = document.getElementById('documents-grid');
            if (dgrid && Array.isArray(data.recommendedDocuments)) {
                dgrid.innerHTML = data.recommendedDocuments.map(doc => docCardHtml(doc)).join('');
                dgrid.querySelectorAll('[data-download-id]').forEach(btn => {
                    btn.addEventListener('click', () => {
                        const id = btn.getAttribute('data-download-id');
                        const item = data.recommendedDocuments.find(d=>d.id===id);
                        if (item) downloadDocumentFake(item);
                    });
                });
            }
        } catch(e) {
    ...
    function docCardHtml(doc){
        ...
        <article class="doc-card">
            ...
            <div class="doc-meta">${doc.pages} pages • ${doc.format}</div>
            <div class="doc-actions">
                <button class="btn-primary" data-download-id="${doc.id}">Download</button>
            </div>
        </article>`;
    }

    function downloadDocumentFake(item){
        const content = `${item.title}\n\nGenerated by Scholarphile. Format: ${item.format}. Pages: ${item.pages}.`;
        const blob = new Blob([content], { type: 'text/plain' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url; a.download = `${item.title.replace(/\s+/g,'-').toLowerCase()}.${item.format.toLowerCase()}`;
